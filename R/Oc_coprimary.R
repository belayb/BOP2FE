...
Oc_coprimary <- function(p1, p2, p3, p4, n_stage, n, nsim, fb, sb, seed = 12345) {
  ...
  for (i in 1:n_stage) {
    ...
    resp[[paste0("EFS6IA", i)]] <- rowSums(resp %>% select(all_of(Y_cols_i13)))
  }
    ...
    resp <- resp %>%
      mutate(
        earlystopfuti = case_when(
          ...
          !!sym(paste0("ORIA", i)) <= fb[2 * (i - 1) + 1] & !!sym(paste0("EFS6IA", i)) <= fb[2 * (i - 1) + 2] ~ 1,
          TRUE ~ earlystopfuti
        ),
        earlystopsupe = case_when(
          earlystopsupe == 1 ~ 1,  # If superiority was already triggered, keep it
          !!sym(paste0("ORIA", i)) >= sb[2 * (i - 1) + 1] | !!sym(paste0("EFS6IA", i)) >= sb[2 * (i - 1) + 2] ~ 1,
          TRUE ~ earlystopsupe
        )
      )
  }
  ...
  for (i in 1:n_stage) {
    ...
    resp <- resp %>%
      mutate(
        cond_1 = !!sym(paste0("ORIA", i)) <= fb[idx1] & !!sym(paste0("EFS6IA", i)) <= fb[idx2],
        cond_2 = !!sym(paste0("ORIA", i)) >= sb[idx1] | !!sym(paste0("EFS6IA", i)) >= sb[idx2],
  ...
  resp <- resp%>%
    mutate(rejectnull := case_when(earlystopsupe == 1 | (earlystopfuti == 0 &
                                                           (!!sym(paste0("ORIA", n_stage)) >= sb[idx1] | !!sym(paste0("EFS6IA", n_stage)) >= sb[idx2])) ~ 1,
                                   TRUE ~ 0))
