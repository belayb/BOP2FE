...
#' @param p1 Response rate (ORR)
#' @param p2 Response rate (DCR - ORR)
#' @param p3 Response rate (1-DCR)
...
Oc_nested <- function(p1, p2, p3, n_stage, n, nsim, fb, sb, seed = 12345) {
  ...
  for (i in 1:n_stage) {
    ...
    resp[[paste0("CRIA", i)]] <- rowSums(resp %>% select(all_of(Y_cols_i1)))
    resp[[paste0("CRPRIA", i)]] <- rowSums(resp %>% select(all_of(Y_cols_i12)))
  }
    ...
    resp <- resp %>%
      mutate(
        earlystopfuti = case_when(
          ...
          !!sym(paste0("CRIA", i)) <= fb[2 * (i - 1) + 1] & !!sym(paste0("CRPRIA", i)) <= fb[2 * (i - 1) + 2] ~ 1,
          TRUE ~ earlystopfuti
        ),
        earlystopsupe = case_when(
          earlystopsupe == 1 ~ 1,  # If superiority was already triggered, keep it
          !!sym(paste0("CRIA", i)) >= sb[2 * (i - 1) + 1] | !!sym(paste0("CRPRIA", i)) >= sb[2 * (i - 1) + 2] ~ 1,
          TRUE ~ earlystopsupe
        )
      )
  }
  ...
  for (i in 1:n_stage) {
    ...
    resp <- resp %>%
      mutate(
        cond_1 = !!sym(paste0("CRIA", i)) <= fb[idx1] & !!sym(paste0("CRPRIA", i)) <= fb[idx2],
        cond_2 = !!sym(paste0("CRIA", i)) >= sb[idx1] | !!sym(paste0("CRPRIA", i)) >= sb[idx2],
  ...
  resp <- resp%>%
    mutate(rejectnull := case_when(earlystopsupe == 1 | (earlystopfuti == 0 &
                                      (!!sym(paste0("CRIA", n_stage)) >= sb[idx1] | !!sym(paste0("CRPRIA", n_stage)) >= sb[idx2])) ~ 1,
                                   TRUE ~ 0))
