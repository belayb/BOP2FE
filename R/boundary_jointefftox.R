...
#' \itemize{
#' \item{cn11f_max: }{Futility boundry for response}
#'  \item{cn11s_min: }{superiority boundry for toxicity}
#'   \item{cn11s_min: }{superiority boundry for response} 
#'   \item{cn12s_min: }{superiority boundry for toxicity} 
#'   } 


boundary_jointefftox <- function(H0, a, n, lambda, gamma, eta=NULL,  method = "power",seed = NULL) {
  ...
  find_boundaries <- function(n, H0, a, cf, cs) {
    ...
    data <- data%>%
      dplyr::mutate(
        ...
        postp12 := calculate_postp2(phi2, beta_a_12, beta_b_12),
        #cf=cf,
        #cs=cs,
        cn11f := dplyr::if_else(postp11 < cf, Y11 + Y12, NA_integer_),
        cn12f := dplyr::if_else(postp12 < cf, Y11 + Y13, NA_integer_),
        cn11s := dplyr::if_else(postp11 >= cs & postp12 >=cs, Y11 + Y12, NA_integer_),
        cn12s := dplyr::if_else(postp12 >= cs & postp12 >=cs, Y11 + Y13, NA_integer_)
      )

    data.frame(
      cn11f_max = max(data$cn11f, na.rm = TRUE),
      cn12f_max = min(data$cn12f, na.rm = TRUE),
      cn11s_min = min(data$cn11s, na.rm = TRUE),
      cn12s_min = max(data$cn12s, na.rm = TRUE)
    )
  }
