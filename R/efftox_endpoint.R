...
get_boundary_jointefftox <- function(H0, a, n, lambda, gamma, eta = NULL, method = "power", seed = NULL) {
  ...
  calculate_postp2 <- function(H0, beta_a, beta_b) {
    pbeta(H0, beta_a, beta_b)
  }
  ...
  generate_nested_sequences <- function(n, a, b1, b2, phi1, phi2) {
    data_list <- lapply(seq_along(n), function(i) {
      ...
      data[["postp12"]] <- mapply(calculate_postp2, phi2, data[["beta_a_12"]], data[["beta_b_12"]])
    ...
    lapply(seq(lambda), function(i) {
      t(
        sapply(seq(gamma), function(j) {
          apply(
            cbind(0, Y12_range * NA ^ (!(postp11_range < cf_values[i, , j]))), 
            1, 
            FUN = max, 
            na.rm = TRUE
          )
  ...
  cnf_max2 <- do.call(
    ...
    lapply(seq(lambda), function(i) {
      t(
        sapply(seq(gamma), function(j) {
          apply(
            cbind(Y13_range * NA ^ (!(postp12_range < cf_values[i, , j])), cum_n), 
            1, 
            FUN = min, 
            na.rm = TRUE
          )
  ...
  if(method == "OF") {
    ...
    cns_min1 <- t(
      sapply(seq(lambda), function(i) {
        apply(
          cbind(Y12_range * NA ^ (!((postp11_range >=cs_values[i, ])&(postp12_range >= cs_values[i, ]))), cum_n), 
          1, 
          FUN = min, 
          na.rm = TRUE
        )
    ...
    cns_min2 <- t(
      sapply(seq(lambda), function(i) {
        apply(
          cbind(0, Y13_range * NA ^ (!((postp11_range >=cs_values[i, ])&(postp12_range >= cs_values[i, ])))), 
          1, 
          FUN = max, 
          na.rm = TRUE
        )
      ...
      lapply(seq(lambda), function(i) {
        t(
          sapply(seq(eta), function(j) {
            apply(
              cbind(Y12_range * NA ^ (!((postp11_range >= cs_values[i, , j])&(postp12_range >= cs_values[i, , j]))), cum_n), 
              1, 
              FUN = min, 
              na.rm = TRUE
            )
    ...
    cns_min2 <- do.call(
      ...
      lapply(seq(lambda), function(i) {
        t(
          sapply(seq(eta), function(j) {
            apply(
              cbind(0, Y13_range * NA ^ (!((postp11_range >=cs_values[i, ,j ])&(postp12_range >= cs_values[i, , j ])))), 
              1, 
              FUN = max, 
              na.rm = TRUE
            )
...
#' @param p1 Response rate (Response - toxicity)
#' @param p2 Response rate (Response - no toxicity)
#' @param p3 Response rate (No Response - toxicity)
#' @param p4 Response rate (No Response - no toxicity)
#' @param n A numeric vector representing the additional patients enrolled at each interim analysis. 
#' The value at index `i` indicates the number of new patients added at interim analysis `i`. 
#' The total sample size at interim `i` is the cumulative sum of the values in `n` up to that index. 
#' For example, for four interim analyses with total sample sizes of 10, 15, 20, and 30, 
...
#' in the following order: c(resp_1,..., resp_{length(n)}, tox_1, ..., tox_{length(n)})
...
get_oc_jointefftox <- function(p1, p2, p3, p4, n, nsim, fb, sb, seed = NULL) {
  ...
  colnames(sum_efs6ia) <- paste0("TOXIA", seq(num_interims))
  ...
  earlystopsupe_OR <- do.call(
    rbind,
    lapply(
      split(
        as.matrix(resp[, paste0('ORIA', seq(num_interims - 1))]), 
        row(as.matrix(resp[, paste0('ORIA', seq(num_interims - 1))]))
      ), 
      function(x) t(x >= t(sb2[, paste0('s1', seq(num_interims - 1))]))
    )
  )
  earlystopsupe_ToX <- do.call(
    rbind,
    lapply(
      split(
        as.matrix(resp[, paste0('TOXIA', seq(num_interims - 1))]), 
        row(as.matrix(resp[, paste0('TOXIA', seq(num_interims - 1))]))
      ), 
      function(x) t(x <= t(sb2[, paste0('s2', seq(num_interims - 1))]))
    )
  )
  
  earlystopsupe <- '*'(earlystopsupe_OR,
                       earlystopsupe_ToX)
  
  fb2 = uniq_fb_and_sb[, c('lambda', 'gamma', paste0('f1', seq(num_interims - 1)), paste0('f2', seq(num_interims - 1)))]
  
  earlystopfuti_OR <- do.call(
    rbind,
    lapply(
      split(
        as.matrix(resp[, paste0('ORIA', seq(num_interims - 1))]), 
      ...
      function(x) t(x <= t(fb2[, paste0('f1', seq(num_interims - 1))]))
    )
  )
  
  earlystopfuti_ToX <- do.call(
    rbind,
    lapply(
      split(
        as.matrix(resp[, paste0('TOXIA', seq(num_interims - 1))]), 
      ...
      function(x) t(x >= t(fb2[, paste0('f2', seq(num_interims - 1))]))
    )
  )
  
  earlystopfuti <- '+'(earlystopfuti_OR,
                       earlystopfuti_ToX)
  ...
  futi_time = do.call(
    function(...) pmin(..., na.rm = TRUE), 
    as.data.frame(
      t(t(earlystopfuti>0) * seq(num_interims - 1)) * (NA ^ (1 - (earlystopfuti>0)))
    )
  )
  ...
  conditionfuti_OR <- do.call(
    rbind,
    lapply(
      split(
        as.matrix(resp[, paste0('ORIA', seq(num_interims))]), 
        row(as.matrix(resp[, paste0('ORIA', seq(num_interims))]))
      ), 
      function(x) t(x <= t(fb21[, paste0('f1', seq(num_interims))]))
    )
  )
  conditionfuti_ToX <- do.call(
    rbind,
    lapply(
      split(
        as.matrix(resp[, paste0('TOXIA', seq(num_interims))]), 
        row(as.matrix(resp[, paste0('TOXIA', seq(num_interims))]))
      ), 
      function(x) t(x >= t(fb21[, paste0('f2', seq(num_interims))]))
    )
  )
  conditionfuti <- '+'(conditionfuti_OR,
                       conditionfuti_ToX)
  
  sb21 = uniq_fb_and_sb[, c('lambda', 'eta', paste0('s1', seq(num_interims)), paste0('s2', seq(num_interims)))]
  conditionsupe_OR <- do.call(
    rbind,
    lapply(
      split(
        as.matrix(resp[, paste0('ORIA', seq(num_interims))]), 
      ...
      function(x) t(x >= t(sb21[, paste0('s1', seq(num_interims))]))
    )
  )
  conditionsupe_ToX <- do.call(
    rbind,
    lapply(
      split(
        as.matrix(resp[, paste0('TOXIA', seq(num_interims))]), 
      ...
      function(x) t(x <= t(sb21[, paste0('s2', seq(num_interims))]))
    )
  )
  
  conditionsupe <- '*'(conditionsupe_OR,
                       conditionsupe_ToX)
      ...
      '*'(
        ...
        t(
          outer(
            ...
            outer(
              resp[[paste0('TOXIA', num_interims)]], sb21[[paste0('s2', num_interims)]], '<=')
        )
      )
    ) > 0,
    ncol = nsim
...
get_boundary_oc_efftox <- function(
  ...
  colnames(all_res_df) <- ifelse(
    ...
    c(paste0('fut_boundary_OR', seq(n)), paste0('fut_boundary_Tox', seq(n))),
    colnames(all_res_df)
  )
  colnames(all_res_df) <- ifelse(
    colnames(all_res_df) %in% c(paste0('s1', seq(n)),paste0('s2', seq(n))),
    c(paste0('sup_boundary_OR', seq(n)),paste0('sup_boundary_Tox', seq(n))),
    colnames(all_res_df)
  )
  return(all_res_df)
}
...
#' 
#' `search_optimal_pars_efftox()` is a helper function and calls `get_boundary_oc_efftox()` 
...
search_optimal_pars_efftox <- function(
  ...
  all_results = get_boundary_oc_efftox(H0, H1, n, nsim, lambda, gamma, eta, method, seed)
