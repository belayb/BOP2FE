...
get_boundary_nested <- function(H0, a, n, lambda, gamma, eta = NULL, method = "power", seed = NULL) {
  set.seed(seed)
  ...
  generate_nested_sequences <- function(n, a, b1, b2, phi1, phi2) {
    data_list <- lapply(seq_along(n), function(i) {
      ...
      data[["beta_a_12"]] <- b2[1] * (a[1] + data[["Y1"]]) + b2[2] * (a[2] + data[["Y2"]]) + b2[3] * (a[3] + data[["Y3"]])
      data[["beta_b_12"]] <- (1 - b2[1]) * (a[1] + data[["Y1"]]) + (1 - b2[2]) * (a[2] + data[["Y2"]]) + (1 - b2[3]) * (a[3] + data[["Y3"]])
  ...
  combine_data <- function(nested_sequences, var) {
    ...
    nested_sequences <- lapply(nested_sequences, function(data) {
      data[!duplicated(data[, c('Y1', 'Y12', 'postp11', 'postp12')]), ]
    })
    ...
    lapply(seq(lambda), function(i) {
      t(
        sapply(seq(gamma), function(j) {
          apply(
            cbind(0, Y1_range * NA ^ (!((postp11_range < cf_values[i, , j])&(postp12_range < cf_values[i, , j])))), 
            1, 
            FUN = max, 
            na.rm = TRUE
          )
  ...
  if(method == "OF") {
    ...
    cns_min1 <- t(
      sapply(seq(lambda), function(i) {
        apply(
          cbind(Y1_range * NA ^ (postp11_range < cs_values[i, ]), cum_n), 
          1, 
          FUN = min, 
          na.rm = TRUE
        )
    ...
    cns_min2 <- t(
      sapply(seq(lambda), function(i) {
        apply(
          cbind(Y12_range * NA ^ (postp12_range < cs_values[i, ]), cum_n), 
          1, 
          FUN = min, 
          na.rm = TRUE
        )
      ...
      lapply(seq(lambda), function(i) {
        t(
          sapply(seq(eta), function(j) {
            apply(
              cbind(Y1_range * NA ^ (postp11_range < cs_values[i, , j]), cum_n), 
              1, 
              FUN = min, 
              na.rm = TRUE
            )
    ...
    cns_min2 <- do.call(
      ...
      lapply(seq(lambda), function(i) {
        t(
          sapply(seq(eta), function(j) {
            apply(
              cbind(Y12_range * NA ^ (postp12_range < cs_values[i, , j]), cum_n), 
              1, 
              FUN = min, 
              na.rm = TRUE
            )
...
#' in the following order: c(CR_1,..., CR_{length(n)}, CR/PR_1, ..., CR/PR_{length(n)})
...
get_oc_nested <- function(p1, p2, p3, n, nsim, fb, sb, seed = NULL) {
  ...
  colnames(resp) <- c("obs", unlist(lapply(1:num_interims, function(i) paste0(c("Y", "Y", "Y"), i, 1:3))))
  
  sum_cria <- t(apply(resp[,paste0("Y", 1:(num_interims), "1")], 1,cumsum))
  sum_crpria <- t(apply(resp[,paste0("Y", 1:(num_interims), "1")], 1,cumsum)) + t(apply(resp[,paste0("Y", 1:(num_interims), "2")], 1,cumsum))
  colnames(sum_cria)<- paste0("CRIA", seq(num_interims))
  colnames(sum_crpria) <- paste0("CRPRIA", seq(num_interims))
  
  resp<- as.data.frame(cbind(resp, sum_cria, sum_crpria))
  ...
  earlystopsupe_temp <- do.call(
    ...
    lapply(
      split(
        as.matrix(resp[, c(paste0('CRIA', seq(num_interims - 1)), paste0('CRPRIA', seq(num_interims - 1)))]), 
        row(as.matrix(resp[, c(paste0('CRIA', seq(num_interims - 1)), paste0('CRPRIA', seq(num_interims - 1)))]))
      ), 
  ...
  # as.matrix earlystopfuti & earlystopsupe so that it should work when we have only one interim analysis 
  ...
  conditionfuti_temp <- do.call(
    ...
    lapply(
      split(
        as.matrix(resp[, c(paste0('CRIA', seq(num_interims)), paste0('CRPRIA', seq(num_interims)))]), 
        row(as.matrix(resp[, c(paste0('CRIA', seq(num_interims)), paste0('CRPRIA', seq(num_interims)))]))
      ), 
  ...
  rejectnull = matrix(
    '+'(
      ...
      '*'(
        ...
        t(
          outer(
            resp[[paste0('CRIA', num_interims)]], sb21[[paste0('s1', num_interims)]], '>=') |
            outer(
              resp[[paste0('CRPRIA', num_interims)]], sb21[[paste0('s2', num_interims)]], '>=')
        )
      )
    ) > 0,
    ncol = nsim
...
get_boundary_oc_nested <- function(
  ...
  boundary_tab <- get_boundary_nested(
  ...
  null_oc <- get_oc_nested(
    p1 = H0[1], p2=H0[2], p3=H0[3], n = n, nsim = nsim, fb = boundary_tab[['cnf']], sb = boundary_tab[['cns']],seed=seed
  )
  ...
  alt_oc <- get_oc_nested(
    p1 = H1[1], p2=H1[2], p3=H1[3], n = n, nsim = nsim, fb = boundary_tab[['cnf']], sb = boundary_tab[['cns']],seed=seed
  )
...
search_optimal_pars_nested <- function(
  ...
  all_results = get_boundary_oc_nested(H0, H1, n, nsim, lambda, gamma, eta, method, seed)
